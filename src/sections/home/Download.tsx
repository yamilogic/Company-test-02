...
const BUTTONS = [
  {
    label: "Download IOS",
    cls: "bg-[#43495B] hover:bg-[#43495B]/85 text-white",
    icon: <path d={APPLE_PATH} />,
  },
  {
    label: "Download Android",
    cls: "border border-zinc-200 bg-white hover:bg-zinc-100 text-zinc-700 hover:bg-zinc-50",
    icon: <path d="M3 20.5v-17c0-.83.94-1.3 1.6-.8l14 8.5c.6.37.6 1.23 0 1.6l-14 8.5c-.66.5-1.6.03-1.6-.8z" />,
  },
];
...
const DownloadSection = () => {
  return (
    <div id="download" className="w-full px-4 sm:px-6 md:px-22 py-4 bg-zinc-100">
      <div className="w-full rounded-3xl bg-white px-6 sm:px-8 md:px-28 py-8 md:py-12 border border-[#E0E2E8] overflow-hidden">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-10 lg:gap-4">
          ...
          <div className="flex flex-col gap-5 w-full md:max-w-[520px] z-10 items-center text-center md:items-start md:text-left">
            ...
            <div className="flex items-center gap-1 sm:gap-2 w-fit bg-[#F3F4F6] rounded-full px-2 sm:px-3 py-1 whitespace-nowrap">
              <span className="text-[#9DA3B7] rounded-full p-1 text-[12px] sm:text-[14px] bg-white font-medium">Achievement</span>
              <div className="w-px h-3 bg-zinc-200" />
              <span className="text-[#9DA3B7] text-[12px] sm:text-[14px] font-medium whitespace-nowrap">Rated No.1 App of 2025</span>
            ...
            <div className="flex flex-col sm:flex-row md:flex-col sm:gap-2 w-full items-center md:items-start">
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-[#43495B] leading-tight tracking-tight">Download Riala Pay</h1>
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-[#43495B] leading-tight tracking-tight">Just <span className="text-[#5A81FA]">Now..!</span></h1>
            </div>
            ...
            <div className="flex flex-col sm:flex-col md:flex-col lg:flex-row items-stretch gap-3 w-full md:w-auto">
              {BUTTONS.map((btn) => (
                <button
                  key={btn.label}
                  className={`flex items-center justify-center gap-2 whitespace-nowrap text-sm font-medium px-3 py-2.5 rounded-xl transition-all w-full md:w-auto ${btn.cls}`}
                >
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" className="shrink-0">{btn.icon}</svg>
          ...
          <div className="relative flex items-center mt-22 justify-center md:justify-end md:w-[45%] w-full h-[240px] sm:h-[300px] md:h-[300px] lg:h-[420px]">
            <Image src={MobileView} alt="mobile" className="md:hidden object-contain w-[260px] sm:w-[320px] h-auto" priority />
            <Image src={Merged} alt="mobile" className="hidden md:block object-contain md:w-[320px] lg:w-[420px] h-auto lg:-translate-x-6" priority />
          </div>
